...
| **P1 真 worker** | 接 LiveAvatar；音频流式化改造 | 🟡 调度算术已完成并对拍上游（55 单测 ＋ 12 变异全杀）；**绑模型待 GPU 机器** |
| **P2 运维** | 配额、指标、压测 | ⬜ 未开始 |
...
### ✅ 已完成：调度算术（`worker/audio_stream.py`）

「架构上不是死路」原先是个判断，现在是**从源码核实过的事实**：

`get_sample_indices(fixed_start=0)` 退化成一条等差斜坡，
**第 i 个槽位取哪一帧只跟 i 有关，跟音频总长无关**（总长只决定有多少槽位、
以及哪些槽位填零）。消费端 `causal_s2v_pipeline_tpp_blockwise.py:843-873`
也确认只往前读，没有一处越过当前 `(r+1)*infer_frames`。

测试里把上游 `get_sample_indices` 用 numpy 原样复刻，**逐点对拍** ——
这条是整个流式改造的立足点，不能只靠读代码下结论。

外部锚点：一轮吃 150 个嵌入帧 ÷ 30 Hz = 5 s，出 80 帧 ÷ 16 fps = 5 s。
两个数走完全不同的公式，对得上说明几何没算歪。

> ⚠️ **有一处有意偏离上游，别「修」回去。**
> 上游 `num_repeat = int(N/150) + 1`，那个 `+1` 恒定多出一轮：N 正好 150 时
> 它给 2 轮，第二轮几乎全是零填充。离线只是浪费，**直播里那是数字人对着
> 空气动 5 秒嘴**。所以流式用 `ceil`。两者的差被测试夹死在 0 或 1 轮，
> 并且同时钉住「不能吞句尾」和「不能多转」——`ceil` 是唯一解。
...
性能数据全部出处见 [优化实录](https://github.com/yangwhale/gpu-tpu-pedia/tree/main/gpu/inference/LiveAvatar/OPTIMIZATION-JOURNAL.md)。
