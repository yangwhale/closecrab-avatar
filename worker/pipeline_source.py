...
所以下面这些**不是待办，是已经放弃的选项**，看到别手痒：

- 这条管线做 batch>1 在结构上不成立：`_initialize_kv_cache(batch_size=1)`
  把 batch 写死了，而且 KV cache / 运动帧 / 注意力锚点 / 音频模板
  **全挂在模型对象上**（`self.kv_cache1`、`self.audio_template` …），
  两路请求会互相踩。要改就得把这些状态从模型对象上摘成每单一份 ——
  动的是上游筋骨。
- 想提并发就**加组**（五卡一组），不是在组内做 batch。
- 真要一卡一用户，那是另一条路（单卡 pipeline），跟这个文件无关。
